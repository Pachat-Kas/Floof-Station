...
public sealed partial class ContestsSystem : EntitySystem
{
    ...
    public float HealthContest(EntityUid performer, bool bypassClamp = false, float rangeFactor = 1f)
    {
        ...
        return ContestClamp(ContestClampOverride(bypassClamp)
            ? 1 - _damageableSystem.GetTotalDamage(performer).Float() / threshold.Value.Float()
            : 1 - Math.Clamp(_damageableSystem.GetTotalDamage(performer).Float() / threshold.Value.Float(), 0, 0.25f * rangeFactor));
    }
    ...
    public float HealthContest(EntityUid performer, EntityUid target, bool bypassClamp = false, float rangeFactor = 1f)
    {
        ...
        return ContestClamp(ContestClampOverride(bypassClamp)
            ? (1 - _damageableSystem.GetTotalDamage(performer).Float() / perfThreshold.Value.Float())
                / (1 - _damageableSystem.GetTotalDamage(target).Float() / targetThreshold.Value.Float())
            : (1 - Math.Clamp(_damageableSystem.GetTotalDamage(performer).Float() / perfThreshold.Value.Float(), 0, 0.25f * rangeFactor))
                / (1 - Math.Clamp(_damageableSystem.GetTotalDamage(target).Float() / targetThreshold.Value.Float(), 0, 0.25f * rangeFactor)));
    }
    #endregion
/*
